...
namespace ISDK.Filuet.OnlineOrdering.NopFiluetCommon.Fusion
{
    public class FusionValidationService
    {
        ...
        public async Task<DistributorLimitsModel> ValidateDistributorLimitsAsync(Customer customer, IList<ShoppingCartItem> shoppingCart, double volumePoints = 0)
        {
            ...
            DistributorFopLimitsModel userLimits = await GetDistributorLimits(customer, true);
            if(userLimits == null)
            {
        ...
        public async Task<DistributorFopLimitsModel> GetDistributorLimits(Customer customer, bool fromCache = false)
        {
            ...
            {
                string cp = await _genericAttributeService.GetAttributeAsync<string>(customer, CustomerAttributeNames.CountryOfProcessing);//settings.CountryCode;// distributorProfile.CountryOfProcessing;
